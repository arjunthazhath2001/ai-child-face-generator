...
class GeneratePresignedURLView(APIView):
    ...
    def post(self,request):
        ...
        object_name = f"uploads/{request.user.clerk_id}/{uuid.uuid4()}/{filename}"
...
class UploadFatherImage(APIView):
    ...
    def post(self,request):
        ...
        father= FatherModel.objects.create(user=request.user,url=file_url)
...
class UploadMotherImage(APIView):
    ...
    def post(self,request):
        ...
        mother= MotherModel.objects.create(user=request.user,url=file_url)
...
class GenerateChildImage(APIView):
    ...
    def post(self,request):
        ...
        try:
            template= FatherModel.objects.get(id=template_id,user=request.user)
            target= MotherModel.objects.get(id=target_id,user=request.user)
        ...
        def task():
            ...
            if url:
                ChildImage.objects.create(user=request.user,url=url,father=template,mother=target) 
            else:
                print("Failed")
...
class CheckChildImage(APIView):
    ...
    def get(self,request,father_id,mother_id):
        try:
            child= ChildImage.objects.get(user=request.user,father_id=father_id,mother_id=mother_id)
